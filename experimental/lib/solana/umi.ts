...
// BeyondFleet achievement tier metadata.
export function createBeyondFleetMetadata(
  ...
  const tierInfo = {
    cadet: { symbol: 'BFCDT', milestone: 'Learning Entry' },
    navigator: { symbol: 'BFNAV', milestone: 'Consistent Learner' },
    pilot: { symbol: 'BFPLT', milestone: 'Research Participant' },
    commander: { symbol: 'BFCMD', milestone: 'Community Contributor' },
    admiral: { symbol: 'BFADM', milestone: 'Long-Term Steward' },
  }

  return {
    name,
    symbol: tierInfo[tier].symbol,
    description: description || `BeyondFleet ${tier.charAt(0).toUpperCase() + tier.slice(1)} achievement record`,
    image: imageUrl,
    attributes: [
      { trait_type: 'Tier', value: tier },
      { trait_type: 'Milestone', value: tierInfo[tier].milestone },
      { trait_type: 'Archive', value: 'BeyondFleet Growth Archive' },
    ],
    properties: {
