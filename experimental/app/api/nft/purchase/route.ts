...
export async function POST(request: NextRequest) {
  try {
    ...
    const { instructions, message, isSimulated, serializedTx } = await createBuyInstruction({
      buyer: buyerWallet,
...
export async function PUT(request: NextRequest) {
  try {
    ...
    // Solana 트랜잭션 확인 (시뮬레이션 트랜잭션이 아닐 때만 실제 온체인 조회)
    const isSimulatedTx = txSignature.startsWith('sim_')
